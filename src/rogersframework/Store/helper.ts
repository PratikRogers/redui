...
export function isAudienceInvokedWithNew(stateObj: any) {
    ...
    if (slickStateObj && getAttrib(slickStateObj, "UserAction") && slickStateObj.UserAction === "SlickPosition" && getAttrib(slickStateObj, "source") && slickStateObj.source === "NEW" && getAttrib(slickStateObj, "slickIdx") && slickStateObj.slickIdx === NavBarConstants.AUDIENCESIZINGSLICK) {
        return true;
    }
    return false;
}
...
export function isAdminListInvokedWithNew(stateObj: any) {
    const slickStateObj = getAttrib(stateObj, "data");
    if (slickStateObj && getAttrib(slickStateObj, "UserAction") && slickStateObj.UserAction === "SlickPosition" && getAttrib(slickStateObj, "source") && slickStateObj.source === "LISTUSER" && getAttrib(slickStateObj, "slickIdx") && slickStateObj.slickIdx === NavBarConstants.ADMINSLICK) {
        return true;
    }
    return false;
} 
...
export function isTraitListInvokedWithNew(stateObj: any) {
    const slickStateObj = getAttrib(stateObj, "data");
    if (slickStateObj && getAttrib(slickStateObj, "UserAction") && slickStateObj.UserAction === "SlickPosition" && getAttrib(slickStateObj, "source") && slickStateObj.source === "LISTTRAITS" && getAttrib(slickStateObj, "slickIdx") && slickStateObj.slickIdx === NavBarConstants.AUDIENCETRAITSLICK) {
        return true;
    }
    return false;
} 
...
export function isInsightsPageInited(stateObj: any) {
    const slickStateObj = getAttrib(stateObj, "data");
    if (slickStateObj && getAttrib(slickStateObj, "UserAction") && slickStateObj.UserAction === "SlickPosition" && getAttrib(slickStateObj, "source") && (slickStateObj.source ===  UserOps.AUDIENCELIST||  slickStateObj.source ===  UserOps.AUDIENCSIZER) && getAttrib(slickStateObj, "slickIdx") && slickStateObj.slickIdx === NavBarConstants.INSIGHTSLICK) {
...
export function isWizardPageStateExist(stateObj: any) {
    const slickStateObj = getAttrib(stateObj, "data") && getAttrib(stateObj.data, "WizardPage") ? stateObj.data.WizardPage : null;
...
export function setAllFalse(stateObj: any) {
    const slickStateObj = getAttrib(stateObj, "data") && getAttrib(stateObj.data, "WizardPage") ? stateObj.data.WizardPage : null;
    if(slickStateObj && slickStateObj.hasOwnProperty("PanelFirstZoneList") && slickStateObj.PanelFirstZoneList.hasOwnProperty("PanelFirstZoneList")) {
        slickStateObj.PanelFirstZoneList.isTraitDeletedInAudienc = false;
        slickStateObj.PanelFirstZoneList.PanelFirstZoneList.forEach(function (item: any) {
            if (item.isSelected) {
...
export function isDashboardChangeAPI(reqObj: any) {
    ...
    if (reqObj.hasOwnProperty("Config") && reqObj.Config.hasOwnProperty("initDashboardCall") && reqObj.Config.initDashboardCall === true) {
        return true;
    }
    return false;
}
...
export function isDashboardToBeInvoked(reqObj: any) {
    if (reqObj.hasOwnProperty("data") && reqObj.data.hasOwnProperty("slickIdx") && reqObj.data.slickIdx === NavBarConstants.DASHBOARDSLICK && reqObj.data.hasOwnProperty("loadAPIData") && reqObj.data.loadAPIData === true) {
        return true;
    }
    return false;
}
...
export function isGeoMapInvoked(reqObj: any) {
    if (reqObj.hasOwnProperty("GeoMapProps") && reqObj.GeoMapProps.hasOwnProperty("initedAPIRequest") && reqObj.GeoMapProps.initedAPIRequest === true) {
        return true;
    }
    return false;
}
...
export function getInitialDashboardObj() {
    ...
    const reqObj = {
        ...
        date: dt + reqDate.getDate() + "-" + mnth + (Number(reqDate.getMonth()+1)) + "-" + reqDate.getFullYear()
    }
...
export function generateDateFromString(dateObj:any) {
    ...
    const date = dt + reqDate.getDate() + "-" + mnth + (Number(reqDate.getMonth()+1)) + "-" + reqDate.getFullYear();
    return date;
}

export function getPlainDate(dateObj:any) {
    
    if(dateObj && dateObj.hasOwnProperty("date") && dateObj.date && dateObj.date.includes("/")) {
        return generateDateFromString(dateObj) ;
    }

    else if(dateObj && dateObj.hasOwnProperty("date") && dateObj.date && !(dateObj.date instanceof Date)) {
        return dateObj.date;
    }
...
export function getDashboardPayload(reqObj: any) {
    ...
    let respObj = reqObj;
    if(reqObj && reqObj.hasOwnProperty("Config") && reqObj.Config.hasOwnProperty("DateConfig") && reqObj.Config.DateConfig.hasOwnProperty("DateSelected") &&  reqObj.Config.DateConfig.DateSelected !== "") {
    ...
    else if(reqObj && reqObj.hasOwnProperty("date") && reqObj.date && reqObj.date instanceof Date) {
        respObj = {
            sourceType: "Age Range",
            date: getPlainDate(reqObj.date)
        }   
    }
    else if(reqObj && reqObj.hasOwnProperty("date") && reqObj.date && reqObj.date.includes("/")) {
        respObj = {
            sourceType: "Age Range",
            date: getPlainDate(reqObj.date)
        }   
...
export function getCity(index:any) {
    let cityArr = ["vancouver", "calgary", "edmonton", "toronto", "ottawa", "montreal"];
...
export function getActionTypeForConfirmOperation(DialogData:any,actionType:any ) {
    if(DialogData.hasOwnProperty("confirmAction") && DialogData.confirmAction) {
        if(DialogData.hasOwnProperty("content") && DialogData.content && DialogData.content.hasOwnProperty("deleteInvokedBy") && DialogData.content.deleteInvokedBy === "AdminUser" ) {
            return UserOps.DELETE_USER; 
        }
        else if(DialogData.hasOwnProperty("content") && DialogData.content && DialogData.content.hasOwnProperty("deleteInvokedBy") && DialogData.content.deleteInvokedBy === "AudienceTrait" ) {
            return UserOps.DELETE_TRAITS; 
        }
        else if (DialogData.hasOwnProperty("content") && DialogData.content && DialogData.content.hasOwnProperty("deleteAudience") && DialogData.content.deleteAudience) {
            return UserOps.DELETE_AUDIENCE_SEGMENT
        }
        else if (DialogData.hasOwnProperty("content") && DialogData.content && DialogData.content.hasOwnProperty("overwriteAudiece") && DialogData.content.overwriteAudiece) {
            return UserOps.OVERWRITE_AUDIENCE;
        }
        
    }
    else if (DialogData.hasOwnProperty("editAudienceAction") && DialogData.editAudienceAction) {
...
export function  isSprtTraitActionInited(stateObj: any) {
    const userObj = getAttrib(stateObj, "userAction");
    if (userObj && userObj === UserOps.TRAIT_MOVE_UP || userObj === UserOps.TRAIT_MOVE_DOWN){
        return true;
    }
    return false;
}
...
export function getTraitSelectedOperaion(stateObj: any,action:any) {
    let returnAction = action;
    if (isAddTraitActionInited(stateObj) || isEditTraitActionInited(stateObj) || isSprtTraitActionInited(stateObj)) {
